...
export function llenarTienda() {
    ...
    let productos = [
            // los arreglos no tenian las comas que los separan 
    ...
    let fila = document.createElementById("fila")//createElementById no existe

    //2. Necesito recorrer un arreglo
    productos.foreach(function () { //falta llamar el arreglo para recorrerlo dentro de la funcion y la E del forEach en mayuscula

        //3. CREAMOS LOS ELEMENTOS NECESARIOS PARA PINTAR LOS PRODUCTOS
        let columna = document.createElement("col")//Se debe utilizar una etiquete de html
        ...
        let imagen = document.createElement("img")//El nombre del arreglo no es igual 
        imagen.classList.add("card-img-top")
        imagen.classList.add("p-3")
        imagen.classList.add("h-100")
        imagen.src = producto.foto// Al llamar la variable tenia mal escrito el nombre ya que era en plural productos, y estaba en singula producto

        let separador = document.getElementById("hr")
        separador.classList.add("w-50")//Para optimizar el codigo se puede separar las clases con coma
        ...
        nombre.textContent = producto.nombre // Al llamar la variable tenia mal escrito el nombre ya que era en plural productos, y estaba en singula producto
        
        let precio = document.createElement("h6")//error de titulo tiene un h6 y llega hasta el h5 
        precio.classList.add("text-center")
        precio.classList.add("text-muted")
        precio.textContent = producto.precio// Al llamar la variable tenia mal escrito el nombre ya que era en plural productos, y estaba en singula producto
